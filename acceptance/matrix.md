...
|---|---|---|---|
| GC-SCALE | 10k tasks, 100k bindings, churn, storage pressure | FAIL | `artifacts/durability-frontier-scale-3bfb8e7aa.json` passes 10,566 logical tasks/124,714 bindings with three deterministic process losses, rollback depths `[24,16,8]`, 104 proof-pruned outputs, bounded disk caches, exact oracle, and drained releases in 1,203.833s. Storage pressure and pruning-disabled comparison remain absent, so the combined row remains FAIL |
...
| GC-MODES | Eight mandatory comparison modes | OPEN | Five DataVine smoke modes pass at `artifacts/grand-challenge-modes-smoke.json`. `artifacts/storage-comparison-070c19e30.json` proves that pruning-on and pruning-off now execute distinct physical policies on the same worker-persistence workflow while preserving correctness. Legacy and accepted-scale all-mode comparison remain absent |
...
| LIGHTWEIGHT | Compact dispatch/queues scale with IDs and bindings | OPEN | Commit `72d06c4fc` dispatches persistent-library calls with TaskID/DataID metadata. `artifacts/grand-challenge-scale-621f2a83a.json` completes 124,201 bindings with 137 EData registrations, but assignment memory bounds are not yet measured |
| SERIAL | Exactly-once serialization and byte-preserving movement | OPEN | A repeated 4 MiB EData object serializes once at `13193c99a`; a 2 MiB IData is serialized/fsynced once per attempt at `53db69f1e`. At accepted normal scale, 124,201 bindings require 137 EData serializations/registrations. Full movement fault proof remains |
...
| CTRL-BOUND | Bounded memory, serving, metadata, queues, cleanup | FAIL | Request/byte admission, bulk bypass, and bounded continuation tombstones pass in earlier artifacts. Commit `3919ab575` bounds completed-lease replay history at 65,536 by default; commit `621f2a83a` makes replica availability snapshot aggregation linear (10k records in 0.026456s; final scale snapshot 0.014093s). General metadata/history cleanup and complete capacity proof remain absent |
...
| SCHED | Independent data progress, minimal rollback, fairness, termination | OPEN | Existing recovery/pruning artifacts prove ordinary-task rollback and independent compute/deletion progress. Commit `ad6ca7c26` prevents release retry starvation by servicing one due obligation after each dispatch; the accepted 10,438-task run retries 63 releases and terminates with zero pending. Failure-scale fairness remains open |
...
| CACHE | Strict DRAM/disk bounds, admission, eviction, zero mode | FAIL | Worker/Manager eviction and zero mode pass earlier tests. At `artifacts/grand-challenge-churn-scale-9468464b4.json`, 10k-scale disk high-water is 17,833,353 bytes/428 items under explicit 64 MiB/2,048-item limits. Incremental accounting observes 10k records in 0.036600s with zero under-capacity candidate scans. DRAM, large one-time bypass, active-transfer eviction, and cleanup remain open |
...
| PERSIST | Bounded/cancellable/backpressured atomic durability | OPEN | Earlier artifacts cover Controller/worker persistence, cancellation, write failure, and admission. `artifacts/storage-comparison-070c19e30.json` persists 786,459 bytes exclusively from workers, applies a configurable 0.1-second quarantine grace, revalidates proof, and leaves no temporary files. Scale I/O limits and remaining races remain open |
| RECOVERY | Replica-aware repeated minimal recovery from frontier | FAIL | `artifacts/durability-frontier-scale-3bfb8e7aa.json` proves three accepted-scale process-loss waves with exact decreasing rollback depths `[24,16,8]`, 48 ordinary attempt-two executions, stable logical IDs, and zero legacy recovery tasks. Multiple-replica loss cases and the full failure matrix remain absent |
...
| PRUNE-SHAREDFS | Quarantine/grace/recovery/hard-delete audit | OPEN | `artifacts/storage-comparison-070c19e30.json` exercises runtime quarantine, configurable grace, fresh-proof hard deletion of two files, zero quarantine leakage, and pruning-off isolation. Quarantine recovery during the grace, pins, restart persistence, and scale remain open |
| MIN-CUT | Observable minimum recoverable cut/frontier/depth | OPEN | Earlier artifacts prove branched cuts and asynchronous proof handling. `artifacts/durability-frontier-scale-3bfb8e7aa.json` advances three checkpoints at 10k-task scale, retires the first two terminal obligations, prunes 104 covered IData records, and records decreasing depths `[24,16,8]`. Recovery-only versus direct-consumer retention and repeated branched churn remain open |
...
| PERF-MGR | Manager/Controller/serialization/metadata metrics | FAIL | The accepted-scale churn artifact records Controller route timings, Manager timing/bytes, 138 EData serializations, 90,472 Scheduler Controller requests, and sampled RSS during the rejected predecessor. Formal CPU/RSS sampling limits and three repetitions remain absent |
| PERF-WORKER | Cache/staging/peer/overlap/idle metrics | FAIL | Partial transfer counts only |
| PERF-FS | Bounded read/write/metadata/storage metrics | FAIL | `artifacts/storage-comparison-070c19e30.json` records identical 786,459-byte worker writes and final retained SharedFS of 262,153 bytes with pruning versus 786,459 bytes without it. Separate read/write concurrency, metadata, peak time series, explicit budget crossing, and scale remain absent |
...
| REPRO | One-command build/regression/challenge and clean runs | OPEN | Exact clean build and 27-test regression pass at `070c19e30`; accepted-scale commands and hashes are recorded in scale artifacts, and the storage A/B has one command in `artifacts/storage-comparison-070c19e30.json`. Factory, clean runtime reset, and all-mode commands remain incomplete |
