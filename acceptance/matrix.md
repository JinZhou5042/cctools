...
| AUTHORITY | Controller sole data/lineage/durability/pruning authority | FAIL | Pruning and physical replica authority absent |
| STATE | Validated logical/physical/durability/recovery/pruning transitions | FAIL | Physical replica and pruning state absent |
| CTRL-BOUND | Bounded memory, serving, metadata, queues, cleanup | FAIL | Bulk bytes retained; HTTP serving and queues not bounded |
| CTRL-FAIL | Auth, idempotency, epochs, stale/partial/restart behavior | FAIL | No worker/replica epochs or restart contract |
