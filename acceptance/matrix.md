...
|---|---|---|---|
| GC-SCALE | 10k tasks, 100k bindings, churn, storage pressure | FAIL | The normal-scale subcondition passes at `artifacts/grand-challenge-scale-621f2a83a.json`: 10,438 logical tasks and 124,201 bindings. `artifacts/repeated-churn-54aaae6fe.json` adds eight ordered process losses at 1k-class scale. Churn at 10k/100k and storage pressure remain absent, so the combined row remains FAIL |
...
| CACHE | Strict DRAM/disk bounds, admission, eviction, zero mode | FAIL | Worker/Manager bounds plus future-used IData eviction/rematerialization pass at `f1237b8b8`. During eight real process losses at `artifacts/repeated-churn-54aaae6fe.json`, disk high-water is 17,831,486 bytes/263 items under explicit 64 MiB/2,048-item limits. DRAM, large one-time bypass, active-transfer eviction, cleanup, and accepted-scale cost remain open |
...
| RECOVERY | Replica-aware repeated minimal recovery from frontier | FAIL | Earlier artifacts prove two-frontier and branch-selective recovery. `artifacts/repeated-churn-54aaae6fe.json` closes eight unique volatile-replica worker processes in order, producing eight ordinary attempt-two recomputations and zero legacy tasks. This 1k-class spine has rollback depth one; accepted-scale churn and the full failure matrix remain absent |
...
| PERF-MGR | Manager/Controller/serialization/metadata metrics | FAIL | The normal-scale artifact records independent Controller route counts/times, Manager timing/bytes, 137 EData serializations, and 88,699 Scheduler Controller requests. CPU/RSS sampling, limits, and three repetitions remain absent |
...
| REPRO | One-command build/regression/challenge and clean runs | OPEN | The clean build, 26-test regression, and exact accepted normal-scale challenge command/commit/hashes are recorded in `artifacts/grand-challenge-scale-621f2a83a.json`. Factory, clean runtime reset, and all-mode commands remain incomplete |
