...
def _expand_legacy_subgraph_dsk(task_dict):
    ...
    appear as SubgraphCallable heads and must be expanded before Workflow conversion.
    """
...
def collections_from_workflow(workflow):
    ...
    for task_key, (func_id, args, kwargs) in workflow.task_dict.items():
        ...
        new_args = workflow._visit_task_output_refs(args, _ref_to_key, rewrite=True)
        new_kwargs = workflow._visit_task_output_refs(kwargs, _ref_to_key, rewrite=True)

        # IMPORTANT: `dask_executor.execute_graph_vertex` expects classic Dask sexprs:
        #   (func, arg1, arg2, ...)
        # It does not understand the Workflow triple (func, args_tuple, kwargs_dict),
...
class Adaptor:
    """Normalize user task inputs so `Workflow` can consume them without extra massaging."""
    ...
    @property
    def task_dict(self):
        """Same as ``converted``: normalized ``{key: (func, args, kwargs)}`` workflow tasks."""
    ...
    def _convert_to_workflow_tasks(self, task_dict):
        """Turn each normalized entry into the `(func, args, kwargs)` triple that Workflow expects."""
        if not task_dict:
            return {}
    ...
    def _convert_dts_graph_node(self, key, node, task_keys):
        """Translate modern Dask TaskSpec graph nodes into workflow expressions."""
        if not dts:
