...
| MIN-CUT | Observable minimum recoverable cut/frontier/depth | OPEN | `artifacts/branched-cut-6135c761a.json` records two durability frontiers, covered IData pruning, a diamond/chain fan-in, and exact recovery wave `[8,7]` while the completed left branch stays at attempt one. Recovery-only versus direct-consumer retention, asynchronous pruning, repeated branched churn, and Grand Challenge scale remain open |
