...
// #define VINE_CHECKPOINT_THRESHOLD (uint64_t)(5 * 1e6) // 10 seconds in usecs
...
struct set *find_file_sources(struct vine_manager *q, const char *cached_name);
...
static int worker_disk_needs_release(struct vine_manager *q, struct vine_worker_info *w)
{
	if (!w || !w->resources || w->is_pbb_worker) {
		return 0;
	}

	int64_t disk_upper_bound = w->resources->disk.total * 90 / 100 * 1024 * 1024;
	int64_t to_release_bytes = w->inuse_cache - disk_upper_bound;

	return to_release_bytes > 0;
}

/* release some space from the worker when the disk is nearly full */
static void clean_one_worker_disk(struct vine_manager *q, struct vine_worker_info *w)
{
	if (!w || !w->resources || w->is_pbb_worker) {
		return;
	}
	...
	if (worker_disk_needs_release(q, w)) {
		HASH_TABLE_ITERATE(w->current_files, cached_name, f)
		{
			if (f->type != VINE_TEMP) {
				continue;
			}
			if (vine_file_replica_count(q, f) > 1) {
				list_push_tail(files_to_delete, f);
			}
		}
	}
	...
	if (worker_disk_needs_release(q, w)) {
		/* try to release space by deleting more temp files */
		HASH_TABLE_ITERATE(w->current_files, cached_name, f)
		{
			if (f->type != VINE_TEMP) {
				continue;
	...
	{
		delete_worker_file(q, w, f->cached_name, 0, 0);
		total_deleted += f->size;
	}

	list_delete(files_to_delete);

	if (total_deleted > 0) {
		debug(D_VINE | D_NOTICE, "Released %lu bytes from worker %s", total_deleted, w->hostname);
	}
}
static void clean_all_workers_disk(struct vine_manager *q)
{
	struct vine_worker_info *w;
	char *key;
	HASH_TABLE_ITERATE(q->worker_table, key, w)
	{
		clean_one_worker_disk(q, w);
	}
}
...
static int handle_cache_update(struct vine_manager *q, struct vine_worker_info *w, const char *line)
{
	...
	if (sscanf(line, "cache-update %s %d %d %lld %lld %lld %lld %s", cachename, &type, &cache_level, &size, &mtime, &transfer_time, &start_time, id) == 8) {
		...
		if (f) {
			...
			/* And if the file is a newly created temporary. */
...
static int handle_cache_invalid(struct vine_manager *q, struct vine_worker_info *w, const char *line)
{
	...
	if (n >= 2) {
		...
		if (n >= 3) {
			...
			struct vine_worker_info *to_worker = vine_current_transfers_get_to_worker(q, transfer_id);
...
static vine_result_code_t get_completion_result(struct vine_manager *q, struct vine_worker_info *w, const char *line)
{
	...
	if (task_status == VINE_RESULT_FORSAKEN) {
		t->forsaken_count++;
		// chances are that the cache-update message arrives just after the file is deleted
		// let's remove the temp input files from the worker
		if (t->forsaken_count > 5){
			struct vine_mount *m;
			LIST_ITERATE(t->input_mounts, m) {
				delete_worker_file(q, w, m->file->cached_name, 0, 0);
			}
		}
		...
		debug(D_VINE, "Library %s failed on worker %s (%s)", t->provides_library, w->hostname, w->addrport);
		if (q->watch_library_logfiles) {
			debug(D_VINE, ", check the library log file %s\n", t->library_log_path);
		} else {
			debug(D_VINE, ", enable watch-library-logfiles for debug\n");
		}
	} else {
...
{
    if (!w || !w->current_files) return;

    int nfiles = hash_table_size(w->current_files);
    if (nfiles < 1) return;

    // 提前缓存 keys（注意 strdup）和 replica 指针
    char **cached_names = malloc(nfiles * sizeof(char *));
    struct vine_file_replica **replicas = malloc(nfiles * sizeof(struct vine_file_replica *));
    ...
    HASH_TABLE_ITERATE(w->current_files, cached_name, replica) {
        cached_names[i] = strdup(cached_name);
        replicas[i] = replica;
        i++;
    }
    ...
    for (i = 0; i < nfiles; i++) {
        ...
        if (!f || !delete_worker_file(q, w, f->cached_name, f->cache_level, VINE_CACHE_LEVEL_WORKFLOW)) {
            if (replica) {
                ...
                if (q->immediate_recovery && f && f->type == VINE_TEMP && f->state == VINE_FILE_STATE_CREATED) {
...
int temp_file_replica_demand(struct vine_manager *q, struct vine_file *f)
{
	...
	return q->temp_replica_count - vine_file_replica_count(q, f);
}

int temp_file_has_ready_source(struct vine_manager *q, struct vine_file *f)
{
	if (!f || f->type != VINE_TEMP) {
		return 0;
	}
...
struct priority_queue *ensure_temp_file_transfer_sources(struct vine_manager *q, struct vine_file *f, int transfer_throttle)
{
	...
	/* If the file doesn't exist, it indicates that the file is either pruned or lost.
	...
	if (!sources) {
		...
		int f_idx = priority_queue_find_idx(q->temp_files_to_replicate, f);
		if (f_idx != -1) {
			priority_queue_remove(q->temp_files_to_replicate, f_idx);
		}
		return NULL;
	}

	/* If all replicas are pending, this file is stuck, so unlink from all workers and remove from the replication queue */
	if (!temp_file_has_ready_source(q, f)) {
		vine_prune_file(q, f);
		if (q->transfer_temps_recovery) {
			vine_manager_consider_recovery_task(q, f, f->recovery_task);
		}
	...
	struct priority_queue *valid_sources = priority_queue_create_with_custom_key(0, vine_worker_key_generator, NULL);
	struct vine_worker_info *w = NULL;
	SET_ITERATE(sources, w)
	{
		...
		if (vine_current_transfers_source_in_use(q, w) >= transfer_throttle) {
			continue;
		}
		struct vine_file_replica *replica = hash_table_lookup(w->current_files, f->cached_name);
		if (!replica) {
		...
		priority_queue_push(valid_sources, w, -(w->resources->disk.total - w->resources->disk.inuse));
	}

	if (priority_queue_size(valid_sources) == 0) {
...
struct priority_queue *ensure_temp_file_transfer_destinations(struct vine_manager *q, struct vine_file *f, int transfer_throttle)
{
	...
	struct priority_queue *valid_destinations = priority_queue_create_with_custom_key(0, vine_worker_key_generator, NULL);
	char *key = NULL;
	struct vine_worker_info *w = NULL;
	HASH_TABLE_ITERATE(q->worker_table, key, w)
	{
		...
		if (vine_current_transfers_dest_in_use(q, w) >= transfer_throttle) {
			continue;
		}
		if (w->is_pbb_worker) {
			continue;
		}
		if (w->resources->disk.total - w->resources->disk.inuse < (long long)f->size) {
			continue;
		}
		priority_queue_push(valid_destinations, w, w->resources->disk.total - w->resources->disk.inuse);
	}
...
static int consider_tempfile_replications(struct vine_manager *q)
{
	...
	struct priority_queue *valid_sources = NULL;
	struct priority_queue *valid_destinations = NULL;

	while (priority_queue_size(q->temp_files_to_replicate) > 0) {
		struct vine_file *f = priority_queue_peek_top(q->temp_files_to_replicate);
		if (!f) {
			return 0;
		}

		valid_sources = ensure_temp_file_transfer_sources(q, f, q->worker_source_max_transfers * 2);
		valid_destinations = ensure_temp_file_transfer_destinations(q, f, q->worker_source_max_transfers * 2);

		if (!valid_sources || !valid_destinations) {
			break;
		}

		int replica_demand = temp_file_replica_demand(q, f);
		if (replica_demand < 0) {
			priority_queue_pop(q->temp_files_to_replicate);
			continue;
		}
		...
		while (priority_queue_size(valid_destinations) > 0) {
			destination = priority_queue_pop(valid_destinations);
			source = NULL;
			int idx;
			int iter_count = 0;
			int iter_depth = priority_queue_size(valid_sources);
			struct vine_worker_info *w = NULL;
			PRIORITY_QUEUE_BASE_ITERATE(valid_sources, idx, w, iter_count, iter_depth)
			{
				if (strcmp(w->hostname, destination->hostname) != 0) {
					source = w;
					break;
				}
			}
		...
		replica_demand = temp_file_replica_demand(q, f);
		if (replica_demand > 0) {
			priority_queue_update_priority(q->temp_files_to_replicate, f, replica_demand);
		} else {
			priority_queue_pop(q->temp_files_to_replicate);
		}

		total_replication_request_sent++;
...
static void recall_worker_lost_temp_files(struct vine_manager *q, struct vine_worker_info *w)
{
	...
	HASH_TABLE_ITERATE(w->current_files, cached_name, info)
	{
		...
		if (f && f->type == VINE_TEMP) {
			// skip if the file is already in the replication queue
			int idx = priority_queue_find_idx(q->temp_files_to_replicate, f);
			if (idx != -1) {
				continue;
			}

			// skip if it does not need replication (if 0, one replica is needed because of this failure)
			int replica_demand = temp_file_replica_demand(q, f);
			if (replica_demand < 0) {
				continue;
			}
...
/* determine if the file is an input to any running tasks on this worker */
/*
static int is_input_to_running_tasks(struct vine_manager *q, struct vine_worker_info *w, struct vine_file *f)
{
	struct vine_task *t;
	ITABLE_ITERATE(w->current_tasks, task_id, t) {
		struct vine_mount *m;
		LIST_ITERATE(t->input_mounts, m) {
			...
			if (strcmp(m->file->cached_name, f->cached_name) == 0) {
				return 1;
			}
		}
	}
...
static void release_worker_disk_space_have_replicas(struct vine_manager *q, struct vine_worker_info *w)
{
	if (!w || !w->resources) {
		return;
	}
...
static void release_worker_disk_space_until_bound(struct vine_manager *q, struct vine_worker_info *w, int64_t disk_upper_bound)
{
	if (!w || !w->resources) {
		return;
	}
	...
	HASH_TABLE_ITERATE(w->current_files, cached_name, f)
	{
		...
		list_push_tail(files_to_delete, f);
		released_bytes += f->size;
		if (released_bytes >= to_release) {
			break;
		}
	}
...
/* release some space from the worker when the disk is nearly full */
static void release_worker_disk_space(struct vine_manager *q, struct vine_worker_info *w)
{
	if (!w || !w->resources) {
		return;
	}

	int64_t disk_upper_bound = (int64_t)(w->resources->disk.total * 0.9);
	if (w->inuse_cache <= disk_upper_bound) {
		return;
	}

	printf("release_worker_disk_space: %s, inuse: %ld, upper_bound: %ld\n", w->addrport, w->resources->disk.inuse, disk_upper_bound);
	/* try to release space by deleting files with more than one replica */
...
static void handle_worker_failure(struct vine_manager *q, struct vine_worker_info *w)
{
	// Prevent removal of PBB worker
	if (w->is_pbb_worker) {
		debug(D_NOTICE, "Attempt to remove the PBB worker %s. PBB workers cannot be removed. Operation cancelled.", w->addrport);
		exit(1);
	}
...
static vine_msg_code_t handle_taskvine(struct vine_manager *q, struct vine_worker_info *w, const char *line)
{
	...
	if (worker_protocol != VINE_PROTOCOL_VERSION) {
		debug(D_VINE, "rejecting worker (%s) as it uses protocol %d. The manager is using protocol %d.", w->addrport, worker_protocol, VINE_PROTOCOL_VERSION);
...
static void reap_task_from_worker(struct vine_manager *q, struct vine_worker_info *w, struct vine_task *t, vine_task_state_t new_state)
{
	...
	if (w->link && link_fd(w->link) >= 0 && link_is_alive(w->link)) {
...
struct vine_manager *vine_ssl_create(int port, const char *key, const char *cert)
{
	...
	*/
	...
	q->checkpoint_queue = hash_table_create(0, 0);
	...
	q->checkpoint_threshold = (uint64_t)(1 * 1e6);

	q->ready_tasks = priority_queue_create_with_custom_key(0, vine_task_key_generator, NULL);
	q->checkpointed_files = priority_queue_create_with_custom_key(0, vine_file_key_generator, NULL);
	q->temp_files_to_replicate = priority_queue_create_with_custom_key(0, vine_file_key_generator, NULL);
	...
	q->pbb_worker = NULL;
	q->kill_num_worker_per_minute = 0;
	q->last_kill_check_time = timestamp_get();
...
void vine_delete(struct vine_manager *q)
{
	...
	hash_table_delete(q->checkpoint_queue);
	...
	priority_queue_delete(q->checkpointed_files);
...
struct vine_task *send_library_to_worker(struct vine_manager *q, struct vine_worker_info *w, const char *name)
{
	...
	*/
	if (original->library_failed_count > q->max_library_retries) {
		...
		debug(D_VINE, "library %s has reached the maximum failure count %d, it has been removed\n", name, q->max_library_retries);
		return 0;
	}
...
void vine_manager_remove_library(struct vine_manager *q, const char *name)
{
	...
	HASH_TABLE_ITERATE(q->worker_table, worker_key, w)
	{
		...
		hash_table_remove(q->library_templates, library_name);
		free(library_name);
...
static int process_checkpoint_queue(struct vine_manager *q)
{
	...
	// vine_checkpoint_check_consistency(q);
	...
	*/
	...
	while ((stoptime == 0) || (time(0) < stoptime)) {
		...
		if (q->wait_for_workers <= hash_table_size(q->worker_table)) {
			...
			if (q->temp_file_checkpoint && q->pbb_worker || !q->temp_file_checkpoint) {
...
int vine_tune(struct vine_manager *q, const char *name, double value)
{
	...
	} else if (!strcmp(name, "temp-file-checkpoint")) {
		q->temp_file_checkpoint = !!((int)value);
...
void vine_prune_file(struct vine_manager *m, struct vine_file *f)
{
	...
	*/
	...
	int f_idx = priority_queue_find_idx(m->temp_files_to_replicate, f);
	if (f_idx != -1) {
		priority_queue_remove(m->temp_files_to_replicate, f_idx);
	}
}
...
void vine_undeclare_file(struct vine_manager *m, struct vine_file *f)
{
	...
	*/
	...
	if (hash_table_lookup(m->file_table, f->cached_name)) {
		char *cached_name = strdup(f->cached_name);
		hash_table_remove(m->file_table, cached_name);
		free(cached_name);
		vine_file_delete(f);
	}
