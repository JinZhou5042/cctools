...
|---|---|---|---|
| GC-SCALE | 10k tasks, 100k bindings, churn, storage pressure | FAIL | `artifacts/grand-challenge-churn-scale-9468464b4.json` passes 10,566 logical tasks/124,714 bindings with eight deterministic process losses, bounded disk caches, exact oracle, and drained releases in 1,215.995s. Storage pressure and pruning comparison remain absent, so the combined row remains FAIL |
...
| RECOVERY | Replica-aware repeated minimal recovery from frontier | FAIL | Accepted-scale churn produces eight ordinary attempt-two recomputations at `9468464b4`. `artifacts/durability-frontier-c7405bcfa.json` adds advancing durable checkpoints and exact decreasing rollback depths `[3,2,1]` while pruning superseded state. The combined frontier schedule is not yet accepted at scale and the full failure matrix remains absent |
...
| MIN-CUT | Observable minimum recoverable cut/frontier/depth | OPEN | Earlier artifacts prove branched cuts and asynchronous proof handling. `artifacts/durability-frontier-c7405bcfa.json` advances three checkpoints, retires the first two terminal obligations, prunes 13 covered IData records, and records decreasing depths `[3,2,1]`. Recovery-only versus direct-consumer retention, repeated branched churn, and Grand Challenge scale remain open |
