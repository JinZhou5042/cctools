...
| RECOVERY | Replica-aware repeated minimal recovery from frontier | FAIL | Earlier artifacts prove two-frontier and branch-selective recovery. `artifacts/grand-challenge-churn-scale-9468464b4.json` closes eight unique volatile-replica worker processes at accepted scale, producing eight ordinary attempt-two recomputations and zero legacy tasks. Rollback depth is one; deep frontier-aware recovery and the full failure matrix remain absent |
