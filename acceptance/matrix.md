...
| PERSIST | Bounded/cancellable/backpressured atomic durability | OPEN | Earlier artifacts cover Controller/worker persistence, cancellation, write failure, and admission. `artifacts/durability-frontier-c7405bcfa.json` distinguishes three historical requests from one live terminal obligation after older checkpoints are quarantined, with no temporary files. Scale I/O limits and remaining races remain open |
...
| REPRO | One-command build/regression/challenge and clean runs | OPEN | Exact clean build, 26-test regression, accepted-scale churn command, commit, and hashes are recorded in `artifacts/grand-challenge-churn-scale-9468464b4.json`. Factory, clean runtime reset, and all-mode commands remain incomplete |
